  ...
  persist(
    (set, get) => ({
      ...
      nextDay: () => {
        ...
        const CLASS_CONFIG = {
          [PlayerClass.Homeless]: { base: 50, cost: 0, leverage: 0.1 },
          [PlayerClass.Worker]: { base: 3200, cost: 2400, leverage: 1.0 },
          [PlayerClass.Middle]: { base: 12000, cost: 7500, leverage: 5.0 },
          [PlayerClass.Capitalist]: { base: 80000, cost: 16000, leverage: 200.0 },
        };
        const config = CLASS_CONFIG[state.currentClass];
        ...
        // 3. 计算薪资
        const salary = calcSalary(config.base, state.san);
        newGold += salary;
