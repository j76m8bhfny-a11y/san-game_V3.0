        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div className="flex flex-col gap-0.5 min-w-[60px]">
               ...
               <div className="w-16 md:w-24 h-1.5 bg-gray-800 rounded-full overflow-hidden border border-white/5">
                  {/* ✅ 使用 sanMedium (70), sanLow (30) 替换硬编码 */}
                  <div className={`h-full transition-all duration-500 ${san > thresholds.sanMedium ? 'bg-purple-500 shadow-[0_0_10px_purple]' : san > thresholds.sanLow ? 'bg-blue-500' : 'bg-cyan-500'}`} style={{ width: `${Math.min(san, 100)}%` }} />
               </div>
            </div>
          </div>
                 ...
                 {addiction > 0 && (
                   <div className="group relative flex flex-col items-center justify-center pt-1">
                      ...
                      <div className="w-8 h-1 bg-gray-800 rounded-full mt-1 overflow-hidden border border-white/10">
                         {/* ✅ 使用 thresholds.addictionHigh 替换 50 */}
                         <div className={`h-full ${addiction > thresholds.addictionHigh ? 'bg-purple-500' : 'bg-purple-800'}`} style={{ width: `${Math.min(addiction, 100)}%` }} />
                      </div>
                   </div>
                 )}
