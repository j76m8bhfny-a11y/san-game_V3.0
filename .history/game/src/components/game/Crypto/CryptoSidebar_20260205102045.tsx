...
export const CryptoSidebar: React.FC = () => {
  ...
  const LEV_CONFIG = marketRules.trading.leverage;

  // 本地交易状态，使用配置初始化
  const [leverage, setLeverage] = useState(LEV_CONFIG.default);
  const [amount, setAmount] = useState(marketRules.trading.defaultAmount);

  const handleOpenAccount = () => {
    // ✅ 修复：使用配置的 FEE 进行判断，不再硬编码 300
    if (gold < FEE) {
      playSfx('sfx_deny');
      return;
    }
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
        ...
        {!crypto.isAccountOpen ? (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-80">
            ...
            {/* ✅ 修复：按钮文案和禁用逻辑统一使用 FEE 变量 */}
            <button 
              onClick={handleOpenAccount}
              disabled={gold < FEE}
              className={`
                px-6 py-2 font-bold text-xs tracking-widest border transition-colors
                ${gold >= FEE 
