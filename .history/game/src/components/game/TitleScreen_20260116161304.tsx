...
const MenuCard: React.FC<MenuCardProps> = ({ 
  ...
  return (
    <motion.button
      ...
      className={`
        relative w-32 h-48 md:w-40 md:h-56 rounded-lg shadow-[0_20px_50px_rgba(0,0,0,0.5)] 
        flex flex-col items-center justify-end pb-4 px-2
        transition-all duration-300 group
        ${color} ${disabled ? 'cursor-not-allowed grayscale brightness-50' : 'cursor-pointer hover:brightness-110'}
      `}
    >
      {/* 内部装饰内容 */}
      <div className="absolute inset-0 pointer-events-none p-2 opacity-80">
        {children}
      </div>

      {/* 标签文字 */}
      <div className={`relative z-10 text-center ${textColor}`}>
        <div className="text-xl md:text-2xl font-black font-sans tracking-tight leading-none">{label}</div>
        <div className="text-[8px] md:text-[10px] font-mono opacity-70 mt-1">{subLabel}</div>
      </div>
