...
export const RoastModal: React.FC = () => {
  ...
  const dismiss = useGameStore(s => s.dismissRoastAndEndEvent); // 👈 使用新方法
        ...
        {currentRoast && (
          ...
          >
            ...
            <button
              onClick={() => {
                playSfx('sfx_click');
                dismiss(); // 👈 点击后才同时关闭吐槽和事件背景
              }}
              className="mt-4 px-8 py-2 bg-white text-black font-bold font-pixel rounded hover:bg-gray-200 active:scale-95 transition-all"
            >
              [ 我知道了 ]
            </button>
