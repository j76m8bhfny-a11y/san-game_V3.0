...
export const useGameStore = create<GameStore>()(
  persist(
    (set, get) => ({
      ...
      shopItems: () => {
        ...
        return gameDataCache.items.filter((item: Item) => {
          ...
          // 可以在这里扩展更多基于 activeEffect 的过滤逻辑
          return true; 
        });
      },
      ...
      nextDay: () => {
        ...
        if (state.day >= global.gameRules.maxDays && state.hp > global.gameRules.victoryHpThreshold) {
            get().triggerEnding('ED-06'); // TODO: 可优化为 resolveEnding 检查 SURVIVAL 类型
            return;
        }
        ...
        if (newHp <= 0) {
            // 死亡判定 (依然通过 resolveEnding 统一处理，但这里先拦截)
            const deathEnding = resolveEnding({ ...state, hp: newHp }, gameDataCache.endings);
      ...
      chooseOption: (optionId) => {
        ...
        // 动态读取 Global Config 计算压力
        const P = calcPressure(state.san, gameDataCache.global.gameRules.pressureDivisor);
        ...
        // 兼容旧逻辑的默认模式推断 (如果 JSON 没写 scaling)
        if (!mode) {
      ...
      buyItem: (itemId) => {
        ...
        // 1. 检查资金
        // 注意：除了特殊 activeEffect 允许透支外，通常不允许负债购买
        ...
        // 3. 应用通用 Active Effects (替代硬编码 ID 判断)
        if (item.activeEffect) {
            const { type, params } = item.activeEffect;
            
            switch (type) {
                case 'SURGERY': // 卖肾/手术类
                    if (newGold < 0) newGold = 0; // 清债
                ...
                case 'BLOOD_DONATION': // 卖血类
                    const gain = params.gold || 40;
                    const cost = params.hpCost || 15;
                    newGold += gain; // 此时 item.price 应该是 0 或负数
                    newHp -= cost;
    ...
    }),
    {
      ...
      version: 13.1, // 版本号升级，触发重置或迁移
      storage: createJSONStorage(() => localStorage),
      migrate: (persistedState: any, version) => {
        if (version !== 13.1) return INITIAL_STATE as any;
        return persistedState;
      },
      onRehydrateStorage: () => (state) => {
        state?.setHydrated();
        // 可以在这里自动触发 initializeData，但通常建议在组件层调用
      },
    }
  )
);