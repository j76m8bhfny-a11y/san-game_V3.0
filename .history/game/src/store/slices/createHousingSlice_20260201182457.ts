...
export const createHousingSlice: StateCreator<any, [], [], HousingSlice> = (set, get) => ({
  ...
  rentHousing: (housingId) => {
    const state = get(); // ✅ 不强制转为 GameState，以便访问 actions
    ...
    // 3. 执行交易
    state.addTransaction('HOUSING', -upfrontCost, `签约租赁: ${house.name}`);
  ...
  buyHousing: (housingId) => {
    ...
    const loanResult = state.takeMortgage(loanAmount, house.buyConfig.mortgageTermTurns, house.buyConfig.interestRate);
