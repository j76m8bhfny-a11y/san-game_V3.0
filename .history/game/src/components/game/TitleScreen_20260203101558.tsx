...
export const TitleScreen: React.FC<TitleScreenProps> = ({ onStart }) => {
  // ✅ 1. 路径修复：day -> vitality.time.currentTurn
  // ✅ 2. 方法名修复：resetGame -> resetPlayerState, nextDay -> nextTurn (或 advanceTurn)
  const { vitality, resetPlayerState, nextTurn } = useGameStore();
  const { playBgm } = useAudioStore();
  
  const currentTurn = vitality.time.currentTurn;
  const hasSave = currentTurn > 0;
