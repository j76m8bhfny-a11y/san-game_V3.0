...
export const DailySettlement: React.FC<DailySettlementProps> = ({ isOpen, onClose }) => {
  // ✨ 1. 从 Store 获取 resetDailyFaith (由 createFaithSlice 提供)
  const { day, gold, hp, san, dailySummary, resetDailyFaith } = useGameStore();
  ...
  // ✨ 2. 处理进入下一天的逻辑
  const handleProceed = () => {
    resetDailyFaith(); // 重置信仰仪式的每日状态 (hasPerformedRite = false)
    onClose();         // 执行父组件传入的关闭/下一天逻辑
  };
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={handleProceed} // ✨ 3. 绑定新的处理函数
