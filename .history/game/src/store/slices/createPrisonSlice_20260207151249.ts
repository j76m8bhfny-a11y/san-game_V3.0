...
export const createPrisonSlice: StateCreator<any, [], [], PrisonSlice> = (set, get) => ({
  ...
  serveTime: () => {
    ...
    if (prison.sentenceTurns <= 0) {
      return { released: true, msg: "刑期异常，已释放", died: false };
    }
    ...
    // 叠加监狱惩罚 (HP/SAN) - 增加空值检查
    const { minStat } = SYSTEM_RULES.caps;
    if (!nextState.vitality?.metrics) {
      return { released: false, msg: "状态异常，无法服刑", died: true };
    }
    nextState.vitality.metrics.hp = Math.max(minStat, nextState.vitality.metrics.hp + effect.hpChange);
    nextState.vitality.metrics.san = Math.max(minStat, nextState.vitality.metrics.san + effect.sanChange);
  ...
  payCashBail: () => {
    ...
    // 边界检查：保释金异常
    if (cost <= 0) return { success: false, msg: "保释金金额异常。" };
  ...
  signBailBond: () => {
    ...
    // 配置验证
    if (rate < 0 || rate > 1) return { success: false, msg: "配置错误：首付比例异常。" };
    if (!loanProductId) return { success: false, msg: "配置错误：缺少贷款产品ID。" };

    const downPayment = Math.floor(totalBail * rate);
    const loanAmount = totalBail - downPayment; // 剩余金额走贷款

    // 边界检查：首付异常
    if (downPayment <= 0) return { success: false, msg: "首付金额异常。" };
    // 检查首付
