    ...
    (set, get) => ({
        ...
        if (bill) {
            ...
            if (bill.effects) {
                if (bill.effects.hp) {
                    ...
                    notes.push(`环境伤害: HP ${bill.effects.hp}`); // 添加到日报
                    log.push(`账单扣血: ${bill.effects.hp}`);      // 添加到历史记录
                }
                if (bill.effects.san) {
                    // 如果未来有扣 SAN 的账单，逻辑写在这里
                    // state.san 需要注意，nextDay 结尾通常不直接 set san，
                    // 但这里可以通过修改 state.san 并在最终 set 时合并
                }
            }
      ...
      chooseOption: (optionId) => {
        ...
        if (optionConfig.roast) {
          ...
          // 3秒后自动消失（可选，也可以让 RoastModal 自己处理点击关闭）
