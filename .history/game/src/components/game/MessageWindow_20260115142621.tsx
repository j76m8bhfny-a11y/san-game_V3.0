...
// 移除 onOptionSelect 属性，直接从 Store 取方法
export const MessageWindow: React.FC<{ event: GameEvent }> = ({ event }) => {
  const san = useGameStore(s => s.san);
  const chooseOption = useGameStore(s => s.chooseOption); // Direct Wiring
    ...
    <div className="absolute bottom-0 w-full h-[40vh] z-10 p-2 md:p-4">
      ...
      <div className="w-full h-full bg-[#020410]/95 border-t-2 border-x-2 border-cyan-800/80 flex flex-col p-4 md:p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.9)] relative overflow-hidden rounded-t-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {options.map((opt, idx) => {
             let btnStyle = "border-cyan-500/40 text-cyan-100 hover:bg-cyan-500 hover:text-black"; 
             ...
             return (
              <button 
                key={opt.id} 
                onClick={() => chooseOption(opt.id)} // 直接调用 Store Action
