...
export const BillOverlay: React.FC<{ bill: Bill }> = ({ bill }) => {
  ...
  useEffect(() => { playSfx('sfx_paper'); }, [playSfx]);

  const handlePay = () => {
    setIsPaying(true);
    playSfx('sfx_cash');
    setTimeout(() => resolveBill(), 800);
  };
  ...
  return (
    <div className="fixed inset-0 z-[6000] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
      <motion.div
        ...
        style={{ clipPath: 'polygon(0 0, 100% 0, 100% 98%, 98% 100%, 0 100%)' }} // 轻微折角
      >
        {/* 顶部装饰条 */}
