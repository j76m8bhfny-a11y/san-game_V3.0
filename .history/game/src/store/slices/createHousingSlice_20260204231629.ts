...
export const createHousingSlice: StateCreator<any, [], [], HousingSlice> = (set, get) => ({
  ...
  // Refactored: 使用配置判断流浪汉允许的区域
  const isHomelessSafeZone = housingRules.access.homelessAllowedRegions.includes(region as string);

  // 如果没有房产，或者在流浪汉允许的区域（如贫民窟），则允许
  if (!activeHousing || isHomelessSafeZone) return true; 
