...
export const checkDailyDisease = (state: GameState, allDiseases: Disease[] = []): string | null => {
  ...
  const diseaseConfig = vitalityRules.disease as any;
  
  // 防御性默认值：防止 JSON 配置缺失导致崩溃
  const infectionRates = diseaseConfig?.infectionRates || { SLUMS: 0.15, RUST_BELT: 0.10, SUBURBS: 0.05, DOWNTOWN: 0.02 };
  const modifiers = diseaseConfig?.modifiers || {
    housingBonus: 0.6,
    homelessPenalty: 1.5,
    jobRiskAdd: 0.05,
    resistanceDivisor: 200,
    weakBodyThresholdPct: 0.3,
    weakSanThresholdPct: 0.2,
    weakBodyMultiplier: 1.5,
    weakSanMultiplier: 1.2
  };
  const highRiskJobs = diseaseConfig?.highRiskJobs || [];
  const distribution = diseaseConfig?.distribution || {};
  ...
  // -- 修正因子 A: 房产保护 (有房产即享受保护) --
  const hasAnyHousing = activeHousing !== null;
  if (hasAnyHousing) {
  ...
  const divisor = modifiers.resistanceDivisor || 200;
  risk *= (1 - metrics.resistance / divisor);
