  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-black font-sans select-none">
      ...
      {/* ==============================
          L1: 主控台 (HUD & Terminal)
         ============================== */}
      <div className={`relative z-10 transition-all duration-500 ${showShop || showArchive ? 'blur-md scale-95 opacity-50' : ''}`}>
        
        {/* 顶部液晶屏: 显示数值 */}
        <MiniHUD 
          day={day} hp={hp} san={san} gold={gold}
        ...
        {!activeBill && currentEvent && (
          <MessageWindow 
            event={currentEvent} 
            onOptionSelect={(optId) => chooseOption(optId)} 
          />
        )}
      </div>
