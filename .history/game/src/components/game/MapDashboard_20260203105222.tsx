...
export const MapDashboard: React.FC = () => {
  // ✅ 1. 修复：从解构中移除根层级的 currentClass，改为获取 vitality
  // ✅ 2. 修复：方法名 attemptMove 修正为 setRegion (对应 PlayerSlice)
  const { 
    currentRegion, 
    vitality, 
    inventory, 
  ...
  const checkUnlock = (region: RegionID) => {
    if (!gameDataCache) return { allowed: false, reason: '数据加载中...' };
            ...
            <button
              ...
              onClick={() => {
                if (isCurrent) {
                  ...
                  // ✅ 3. 调用更新后的 Action
                  setRegion(regionId);
                  setViewMode('REGION'); // 移动后通常直接切换视角
                }
              }}
