  ...
  useItem: (itemId) => {
    ...
    if (item.type === ItemType.CONSUMABLE) {
      ...
      if (item.tags.includes('DRUG') && finalHpGain > 0) {
        ...
        // 简单算法：每 10 点耐药性，药效衰减 10%
        // resistance 50 -> 衰减 50% -> 效果 * 0.5
        const effectiveness = Math.max(0.1, 1 - (currentResistance / 100));
        
        const originalHp = finalHpGain;
        finalHpGain = Math.floor(finalHpGain * effectiveness);

        if (effectiveness < 0.8) {
           notificationMsg += ` (耐药性导致效果衰减)`;
        }
        if (effectiveness < 0.3) {
           notificationMsg += ` (几近失效)`;
        }
      }
      ...
      state.modifyStats({
          ...
          san: effects.san,
          maxHp: effects.maxHp,
          addiction: effects.addiction,   // ✅ 增加成瘾
          resistance: effects.resistance  // ✅ 增加耐药
      });
