...
export const CapitalistInsuranceCard: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const plan = (insuranceData as any[]).find(i => i.id === 'INS_GLOBAL_VIP'); // 假设只有一个顶级保险
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div className="flex justify-between items-end border-t border-[#d4af37]/20 pt-6">
          <div>
             ...
             >
                {isActive ? "Sir. Pixel Player" : "Sign Here..."}
             </button>
          </div>

          <div className="text-right">
            <p className="text-[#d4af37] font-serif text-lg">{isActive ? 'ACTIVE' : 'INVITATION ONLY'}</p>
