...
export interface Item {
  ...
  unlockCondition?: string; // 解锁条件 (如 "Gold < -2000")
  effects: {
    hp?: number;
    san?: number;
    maxHp?: number;
...
export interface GameState {
  ...
  ending: string | null; // 结局 ID
  
  // 玩家资产
  inventory: string[]; // 存 Item ID
  history: string[];   // 历史记录 log
  unlockedArchives: string[]; // 已解锁的 Archive ID

  // 特殊标记 flags
  flags: {
    isHomeless: boolean;
    debtDays: number; // 连续负债天数 (用于触发人体拆解)
    hasRedBook: boolean;
  ...
  // 内部状态
  _hasHydrated: boolean; // 用于持久化加载判断
  
  // 最近一次动作类型 (用于 UI 反馈)
  lastAction: {
    type: 'NONE' | 'INCOME' | 'PAYMENT' | 'DAMAGE' | 'HEAL' | 'INSANITY';
    id: number; // 时间戳，用于触发 useEffect
  };
