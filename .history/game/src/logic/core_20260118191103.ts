...
export const triggerBill = (
    ...
    const cond = bill.triggerCondition;
    if (!cond) return true; // 如果没有条件，视为符合

    if (cond.isDebtOnly && gold >= 0) return false;
    // 使用 optional chaining ?. 安全访问
    if (cond.requiredClass && !cond.requiredClass.includes(currentClass)) return false;
    if (cond.minGold !== undefined && gold < cond.minGold) return false;
