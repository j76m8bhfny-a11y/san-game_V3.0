...
export const InventorySidebar: React.FC = () => {
  ...
  const myItems = inventoryIds.map(id => allItems.find(i => i.id === id)).filter(Boolean);
                ...
                myItems.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="mt-2 pt-2 border-t border-dashed border-cyan-900/50 flex gap-3 text-[10px] text-gray-500 font-sans">
                       {item!.effects.hp !== 0 && <span>HP: {item!.effects.hp > 0 ? '+' : ''}{item!.effects.hp}</span>}
                       {item!.effects.san !== 0 && <span>SAN: {item!.effects.san > 0 ? '+' : ''}{item!.effects.san}</span>}
                    </div>
                  </div>
                ))
              )}
